    ...
    <>
      <header className="fixed z-40">
        <div className="min-h-14 bg-btn-bg/70 backdrop-blur-[100px] shadow-header rounded-xl fixed inset-x-4 xs:inset-x-10 lg:inset-x-20 max-w-[1365px] mx-auto top-4 lg:top-10 z-50 py-3 px-5 flex items-center justify-between xl:justify-normal">
          ...
          {/* Desktop navigation */}
          <div className="hidden xl:block bg-bg-light/30 w-0.5 h-5 ml-5 mr-1" />
                ...
                return (
                  <li key={index} className="group">
                    <LinkButton to={link.href} type="internal" variant="ghost">
                      <div className="bg-btn-bg border border-btn-border rounded-sm p-1 group-hover:motion-preset-seesaw motion-loop-once">
                        {link.icon}
                      </div>
                      <span>{link.text}</span>
                    </LinkButton>
          ...
          <button
            className="bg-btn-bg border border-btn-border p-1 rounded-sm cursor-pointer xl:hidden"
                    ...
                    return (
                      ...
                      >
                        <LinkButton
                          ...
                          variant="default"
                        >
                          <div className="bg-btn-bg border border-btn-border rounded-sm p-1">
                            {link.icon}
                          </div>
                          <span>{link.text}</span>
                        </LinkButton>
      ...
      <footer className="px-4 xs:px-10 lg:px-20 w-full max-w-[1365px] mx-auto pb-28 flex flex-col xl:flex-row xl:justify-between">
        <Link to="/" className="group flex items-center gap-2 self-start">
        ...
        <address className="flex flex-col gap-2 mt-8 xl:mt-0 xl:flex-row xl:gap-4 not-italic self-start">
          ...
          <ul className="flex flex-col gap-2 xl:flex-row xl:gap-4 ">
            {SOCIALS.map((s, index) => {
              return (
                <li key={index} className="group self-start">
                  <a
                    href={s.href}
                    target="_blank"
